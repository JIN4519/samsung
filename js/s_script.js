...
document.addEventListener("DOMContentLoaded", function () {
  // --- Custom Recommend Slider 1 (컨텐츠 5) ---
  ...
  const cardWidth1 = cards1.length > 0 ? cards1[0].offsetWidth + 24 : 220;
  ...
  function updateCustomSlider1() {
    ...
    fill1.style.width = maxIndex1 === 0 ? '95%' : `${(index1 / maxIndex1) * 95}%`;
...
const loginImgMobile = document.getElementById("loginimgmobile");
loginImgMobile?.addEventListener("click", function (e) {
  e.preventDefault();
  e.stopPropagation();
  const loginPanel = document.getElementById("loginpanel");
  loginPanel.classList.toggle("open");
...
prev.onclick = () => {
  if (index > 0) {
    index--;
    updateCustomSlider();
// --- Custom Recommend Slider 2 (복제 영역) ---
// --- Custom Recommend Slider 2 (복제 영역) ---
window.addEventListener('DOMContentLoaded', function() {
  ...
  let index1 = 0;
  const visible1 = 4;
  const cards1 = track1 ? track1.querySelectorAll('.custom-recommend-card') : [];
  const cardWidth1 = cards1.length > 0 ? cards1[0].offsetWidth + 24 : 220;
  const maxIndex1 = cards1.length - visible1 >= 0 ? cards1.length - visible1 : 0;
...
// 데스크탑 로그인/카트 아이콘 색상 자동 변경 (흰색 배경에서는 항상 검정)
const loginImg = document.getElementById('loginimgdesktop');
const cartImg = document.getElementById('cartimgdesktop');
function setDesktopIcons(color) {
  loginImg.src = color === 'black' ? 'image/login_black.png' : 'image/login_white.png';
  cartImg.src = color === 'black' ? 'image/cart_black.png' : 'image/cart_white.png';
}

function updateDesktopIconsByDropdown() {
  // 드롭다운 패널이 보이면 검정, 아니면 흰색
  const dropdowns = [
    document.getElementById('dropdownpanel'),
    document.getElementById('promopanel'),
    document.getElementById('aipanel'),
    document.getElementById('customerpanel')
  ];
  const isAnyDropdownVisible = dropdowns.some(panel => {
    return panel && window.getComputedStyle(panel).display !== 'none';
  });
  setDesktopIcons(isAnyDropdownVisible ? 'black' : 'white');
}

['menutoggle', 'promotoggle', 'aitoggle', 'customertoggle'].forEach(id => {
  const el = document.getElementById(id);
  el.addEventListener('mouseenter', updateDesktopIconsByDropdown);
  el.addEventListener('mouseleave', updateDesktopIconsByDropdown);
});
['dropdownpanel', 'promopanel', 'aipanel', 'customerpanel'].forEach(id => {
  const panel = document.getElementById(id);
  panel.addEventListener('mouseenter', updateDesktopIconsByDropdown);
  panel.addEventListener('mouseleave', updateDesktopIconsByDropdown);
});